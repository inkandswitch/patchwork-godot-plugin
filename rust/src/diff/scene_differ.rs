...
impl Differ<'_> {
    ...
    pub(super) fn get_text_resource_diff(
        &self,
        path: &String,
        old_scene: Option<&GodotScene>,
        new_scene: Option<&GodotScene>,
        ...
        for sub_resource_id in &sub_resource_ids {
            ...
            let Some(diff) = self.get_sub_resource_diff(sub_resource_id, old_sub_resource, new_sub_resource, old_scene, new_scene)
            else {
                // If the node has no changes or is otherwise invalid, just skip this one.
                continue;
            };
